...
var Detector = function(){
	...
	return {
		...
		,isChrome : (/Chrome/i).test(ua) //稳健_已使用
		...
		,isSamSungPad : window.devicePixelRatio == 1 && !isIOS //不穩健_已使用 Android下的UC等瀏覽器也可能與此判斷匹配
...
var getDeviceWidth_cssPixels = function(){
  ...
  if( Detector.isChrome ){
  	  ...
  	  /* 网页选项卡处于显示状态下，转屏screen.width读数正确
	...
	//window.onload = function(cb){
		...
		if(	true ){ // !Detector.isDesktop
			...
			if( !Detector.isDesktop && Detector.isFirefox ){ 
				...
				window.addEventListener( orientationEvent, function(){ fitSize( options ); }, false);
			}
